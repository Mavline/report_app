...
function App() {
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    newFiles.forEach((file) => {
      ...
      reader.onload = (e) => {
        ...
        // Не выбираем лист автоматически, даже если он один
  ...
  const handleSheetSelection = (fileName: string, sheetName: string) => {
    ...
    reader.onload = (e) => {
      ...
      const partialJson = XLSX.utils.sheet_to_json(worksheet, {
        range: tempRange,
        header: 1,
      }) as Array<Array<any>>;
  ...
  const mergeTables = () => {
    ...
    const orderedFields = Object.entries(selectedFields).reduce(
      (acc: string[], [_, fields]) => {
    ...
    for (let i = 1; i < tables.length; i++) {
      ...
      if (!currentFile || !previousFile) {
        alert(
          "Error: Some files are missing. Please upload all required files.",
        );
        return;
      }
      ...
      if (!currentKeyField || !previousKeyField) {
        alert(
          "Error: Key fields are not selected for all files. Please select key fields for all files.",
        );
        return;
      }
      ...
      merged = merged.flatMap((row: any) => {
        const matchingRows = tables[i].filter(
          (r: any) => r[currentKeyField] === row[previousKeyField],
        );
                ...
                <div className="mb-4" style={{ width: "100%" }}>
                  ...
                  >
                    ...
                    {sheets[files[index].name].map((sheet, sheetIndex) => (
                      <option key={`${sheet}-${sheetIndex}`} value={sheet}>
                        {sheet}
                      </option>
                    ))}
                  </select>
